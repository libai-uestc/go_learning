...
func Log(logger *log.Logger) {
	logger.Printf("%d+%d=%d\n", 3, 4, 3+4)
	logger.Println("Hello 李白")
	// logger.Fatalln("Bye!") // 日志输出后会执行os.Exit(1)

	log.Printf("%d+%d=%d\n", 3, 4, 3+4)
	log.Println("Hello 李白")
	// log.Fatalln("bye")
}
		...
		&SlogContextHandler{
			slog.NewJSONHandler(fout, &slog.HandlerOptions{
				...
				ReplaceAttr: func(groups []string, a slog.Attr) slog.Attr {
					// check that we are handing the time key
...
func SLog(logger *slog.Logger) {
	...
	logger.Error("hello libai")

	ctx1 := AppendCtx(context.Background(), slog.String("user", "李白"))
