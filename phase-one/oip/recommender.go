...
import (
	...
	"libai/go/basic/phase-one/oip/filter"
	"libai/go/basic/phase-one/oip/recall"
	"libai/go/basic/phase-one/oip/sort"
	"log"
	"time"
)
...
func (rec *Recommender) Rec() []*common.Product {
	...
	for _, recaller := range rec.Recallers {
		...
		for _, product := range products {
			RecallMap[product.Id] = product // 把多路召回的结果放到一个map里，按Id进行重排
		}
	...
	for _, procudt := range RecallMap {
		RecallSlice = append(RecallSlice, procudt)
	}
	// 对召回的结果进行排序
	begin := time.Now()
