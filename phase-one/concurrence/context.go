...
func InheritTimeout() {
	...
	child, cancel2 := context.WithTimeout(parent, time.Millisecond*1000) // parent还剩500ms，child设置了1000ms之后到期，child.Done()管道关闭时刻以较早的为准，即500ms后到期
	// child,cancel2 := context.WithTimeout(parent,time.Millisecond*1000) // parent还剩500ms，child设置了100ms之后到期，child.Done()管道关闭时刻以较早的为准，即100ms后到期
	t1 := time.Now()
	defer cancel2()
...
func RoutineID() {
	for i := 0; i < 3; i++ {
		...
		ctx = context.WithValue(ctx, StringKey("owner"), StringValue("libai"))
...
func step1(ctx *context.Context) {
	...
	*ctx = context.WithValue(*ctx, StringKey("name"), StringValue("乘风破浪会有时"))
}
